function [Fx_ext,Fy_ext,Lz_ext,Sij_ext,Fxs,Fys,Lzs,Sijs,xs,ys,ux,uy,fx,fy,Mij,Vxs,Vys,Omegas,Vx_ext,Vy_ext,Omega_ext] = regstokes_shape_function_compute_V_Omega(lsd,epsilonfactor,spacings,vext,cmloc,rotationangle,shape_function)
...
for s = 1:length(spacings)
    ...
    %[xs,ys] =hex_packed_diamond(spacing,2*R,rotationangle);
    [xs,ys] = shape_function(spacing,R);
    xs = xs-mean(xs);  % watch out - this part is super sensitive if there is a zero rotation... 
    ys = ys-mean(ys);

    % do a rotation to angle...
    [xs,ys] = rotate_pos(xs,ys,rotationangle);
